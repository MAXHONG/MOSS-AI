...
MOSS AI is a LangGraph-based AI super agent system with a full-stack architecture. The backend provides a "super agent" with sandbox execution, persistent memory, subagent delegation, and extensible tool integration - all operating in per-thread isolated environments.
...
│   │   └── client.py          # Embedded Python client (MOSS AIClient)
│   ├── tests/                 # Test suite
│   └── docs/                  # Documentation
...
Bridges external messaging platforms (Feishu, Slack, Telegram) to the MOSS AI agent via the LangGraph Server.
...
Both can be modified at runtime via Gateway API endpoints or `MOSS AIClient` methods.

### Embedded Client (`src/client.py`)

`MOSS AIClient` provides direct in-process access to all MOSS AI capabilities without HTTP services. All return types align with the Gateway API response schemas, so consumer code works identically in HTTP and embedded modes.
