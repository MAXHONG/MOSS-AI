...
This document describes how to enable and use the Plan Mode feature with TodoList middleware in MOSS AI 2.0.
...
MOSS AI uses custom `system_prompt` and `tool_description` for the TodoListMiddleware that match the overall MOSS AI prompt style:

### System Prompt Features
- Uses XML tags (`<todo_list_system>`) for structure consistency with MOSS AI's main prompt
...
- TodoList middleware uses LangChain's built-in `TodoListMiddleware` with **custom MOSS AI-style prompts**
...
- Custom prompts emphasize the same principles as MOSS AI's main system prompt (clarity, action-oriented, critical rules)
