...
This guide explains how to configure MOSS AI for your environment.
...
MOSS AI supports multiple sandbox execution modes. Configure your preferred mode in `config.yaml`:
...
When using Docker development (`make docker-start`), MOSS AI starts the `provisioner` service only if this provisioner mode is configured. In local or plain Docker sandbox modes, `provisioner` is skipped.
...
MOSS AI supports environment variable substitution using the `$` prefix:
...
MOSS AI searches for configuration in this order:
